...
def propagate(coef, intercept, X, y, lambda_):
    """
    Calculates the gradients and cost for the `RidgeRegression` model.
    Args:
    ...
    cost = (np.sum((y_hat - y) ** 2) + lambda_ * np.sum(coef**2)) / (2 * num_samples)
    dM = (
        np.dot(X.T, (y_hat - y)) + lambda_ * coef
    ) / num_samples  # Gradient of coefficients.
...
class RidgeRegression:
    """
    A ridge regression model for fitting a line to data points with L2 regularisation.

    Attributes:
        coef_ (numpy.ndarray): Coefficients of the ridge regression model.
        intercept_ (float): Intercept of the ridge regression model.
        lambda_ (float): Regularisation parameter.

    Methods:
        fit(X, y): Fit the ridge regression model to the training data.
    ...
    def __init__(self, lambda_=1.0):
        """
        Initialise the RidgeRegression object.
    ...
    def fit(self, X, y, num_iterations=1000, learning_rate=0.01):
        ...
        if X.shape[0] != len(y):
            raise ValueError("The number of samples in X and y don't match.")
    ...
    def score(self, X, y):
        """
        Calculate the coefficient of determination (R^2) for the ridge regression model.
