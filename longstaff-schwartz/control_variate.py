...
Here, the usage of control variates is incorporated for variance reduction
method.
...
def longstaff_schwartz(
    spot, strike, sigma, T, r, N=5000, M=5000, isCall=True, k=3, seed=42
):
    r"""
    ...
    # Initialising the control variate
    control_variate = np.zeros((M, N + 1))

    control_variate[:, 0] = spot  # Initial spot at t=0
    ...
    # Generating the control variate (equivalent martingale measure)

    control_variate[:, 1:] = spot * np.cumprod(np.exp(r * dt * np.ones((M, N))), axis=1)
    ...
    # Calculating the coefficient for the control variate
    average_option_path = np.mean(option_prices, axis=1)
    average_stock_path = np.mean(stock_paths, axis=1)

    # Calculating the `beta`
    beta = np.cov(average_stock_path, average_option_path)[0, 1] / np.var(
        average_stock_path
    )

    print(beta)
    # Subtracting the control variates from the option prices

    option_prices -= beta * (stock_paths - control_variate)
    ...
    mean_option_price = np.mean(cash_flows) * np.exp(-r * dt)

    # Assuming 95% confidence interval
    upper_limit = mean_option_price + 1.96 * np.std(cash_flows) / np.sqrt(M)
    lower_limit = mean_option_price - 1.96 * np.std(cash_flows) / np.sqrt(M)
    return {
